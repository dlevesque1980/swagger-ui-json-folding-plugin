...
export class JsonViewer extends LitElement {
    ...
    *search(criteria: string) {
    ...
    render() {
        ...
        const objectOpener = Array.isArray(data) ? html`<div class="separator">[</div>`:html`<div class="separator">{</div>`;
        const objectCloser = Array.isArray(data) ? html`<div class="separator">]</div>`:html`<div class="separator">}</div>`;
        return isDefined(data) ? html`${objectOpener}${this.renderNode(data)}${objectCloser}` : null;
    }
}
